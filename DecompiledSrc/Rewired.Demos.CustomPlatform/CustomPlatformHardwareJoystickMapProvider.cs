...
public class CustomPlatformHardwareJoystickMapProvider : IHardwareJoystickMapCustomPlatformMapProvider
{
	...
	public Platform GetPlatformMap(int customPlatformId, Guid hardwareTypeGuid)
	{
	...
	private static Platform GetPlatformMap(CustomPlatformHardwareJoystickMapPlatformDataSet platformDataSet, Guid hardwareTypeGuid)
	{
		...
		for (int i = 0; i < count; i++)
		{
			if ((UnityEngine.Object)(object)platformDataSet.platformMaps[i] != null && platformDataSet.platformMaps[i].Matches(hardwareTypeGuid))
			{

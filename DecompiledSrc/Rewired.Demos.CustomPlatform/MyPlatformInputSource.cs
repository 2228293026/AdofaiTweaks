...
{
	public sealed class Joystick : Joystick, IControllerVibrator
	{
			...
			: base(sourceJoystick.deviceName, sourceJoystick.systemId, sourceJoystick.axisCount, sourceJoystick.buttonCount)
		{
			...
			((Controller)this).customIdentifier = _sourceJoystick.identifier;
			((Controller)this).deviceInstanceGuid = sourceJoystick.deviceInstanceGuid;
		}

		public override void Update()
		{
			for (int i = 0; i < ((Joystick)this).buttonCount; i++)
			{
				((Joystick)this).SetButtonValue(i, sourceJoystick.GetButtonValue(i));
			}
			for (int j = 0; j < ((Joystick)this).axisCount; j++)
			{
				((Joystick)this).SetAxisValue(j, sourceJoystick.GetAxisValue(j));
			}
		}
	...
	public MyPlatformInputSource(CustomPlatformConfigVars configVars)
		...
		{
			unifiedKeyboardSource = (CustomPlatformUnifiedKeyboardSource)(object)new MyPlatformUnifiedKeyboardSource(),
			unifiedMouseSource = (CustomPlatformUnifiedMouseSource)(object)new MyPlatformUnifiedMouseSource()
		})
	{
	...
	{
		((CustomInputSource)this).OnInitialize();
		_initialized = true;
	...
	{
		IList<Joystick> joysticks = ((CustomInputSource)this).GetJoysticks();
		...
		for (int num = joysticks.Count - 1; num >= 0; num--)
		{
			...
			{
				((CustomInputSource)this).RemoveJoystick((Joystick)(object)joystick);
			}
		}
		for (int i = 0; i < joysticks2.Count; i++)
		{
			...
			if (!ContainsJoystickBySystemId(joysticks, joystick2.systemId))
			{
				...
				{
					((Joystick)joystick3).extension = (Extension)(object)new MyPlatformControllerExtension(joystick3);
				}
				((CustomInputSource)this).AddJoystick((Joystick)(object)joystick3);
			}
		}
	}
	...
	protected override void Dispose(bool disposing)
	{
		...
		{
			_disposed = true;
			((CustomPlatformInputSource)this).Dispose(disposing);
		}
	}

	private static bool ContainsJoystickBySystemId(IList<Joystick> joysticks, long systemId)
	{

...
public class CUIColorPicker : MonoBehaviour
{
	...
	private void Setup(Color inputColor)
	{
		...
		Action applySaturationValue = delegate
		{
			...
			color5 = (result.GetComponent<Image>().color = new Color(color5.r, color5.g, color5.b, inputColor.a));
			if (_color != color5)
			{
		...
		Action idle = delegate
		{
			...
			{
				if (GetLocalMouse(hueGO, out var vector))
				{
					_update = dragH;
				}
				else if (GetLocalMouse(satvalGO, out vector))
				{
					_update = dragSV;
				}
			}
		...
		dragH = delegate
		{
			GetLocalMouse(hueGO, out var vector);
		...
		dragSV = delegate
		{
			GetLocalMouse(satvalGO, out var vector);

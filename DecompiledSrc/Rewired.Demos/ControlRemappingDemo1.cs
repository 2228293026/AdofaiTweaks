...
public class ControlRemappingDemo1 : MonoBehaviour
{
	...
	private void DrawJoystickSelector()
	{
		...
		if (selectedPlayer != null)
		{
			...
			{
				flag2 = selectedPlayer.controllers.ContainsController((Controller)(object)joystick);
				flag3 = GUILayout.Toggle(flag2, ((Controller)joystick).name, "Button", GUILayout.ExpandWidth(expand: false));
				if (flag3 != flag2)
				{
					EnqueueAction(new JoystickAssignmentChange(selectedPlayer.id, ((Controller)joystick).id, flag3));
				}
			}
		}
		GUILayout.EndHorizontal();
	...
	private void DrawControllerSelector()
	{
		...
		bool flag2 = (int)selectedController.type == 0;
		if (GUILayout.Toggle(flag2, ((Controller)ReInput.controllers.Keyboard).name, "Button", GUILayout.ExpandWidth(expand: false)) != flag2)
		{
			selectedController.Set(0, (ControllerType)0);
			ControllerSelectionChanged();
		}
		...
		flag2 = (int)selectedController.type == 1;
		if (GUILayout.Toggle(flag2, ((Controller)ReInput.controllers.Mouse).name, "Button", GUILayout.ExpandWidth(expand: false)) != flag2)
		{
		...
		{
			flag2 = (int)selectedController.type == 2 && selectedController.id == ((Controller)joystick).id;
			if (GUILayout.Toggle(flag2, ((Controller)joystick).name, "Button", GUILayout.ExpandWidth(expand: false)) != flag2)
			{
	...
	private void DrawCalibrateButton()
	{
		...
		else if (GUILayout.Button("Calibrate " + val.name, GUILayout.ExpandWidth(expand: false)))
		{
	...
	private void DrawMapCategories()
	{
		...
		foreach (InputMapCategory userAssignableMapCategory in ReInput.mapping.UserAssignableMapCategories)
		{
			...
			bool flag2 = ((((InputCategory)userAssignableMapCategory).id == selectedMapCategoryId) ? true : false);
			if (GUILayout.Toggle(flag2, (((InputCategory)userAssignableMapCategory).descriptiveName != string.Empty) ? ((InputCategory)userAssignableMapCategory).descriptiveName : ((InputCategory)userAssignableMapCategory).name, "Button", GUILayout.ExpandWidth(expand: false)) != flag2)
			{
				selectedMapCategoryId = ((InputCategory)userAssignableMapCategory).id;
				selectedMap = selectedPlayer.controllers.maps.GetFirstMapInCategory(selectedController.type, selectedController.id, ((InputCategory)userAssignableMapCategory).id);
			}
	...
	private void DrawCalibrationWindow(string title, string message)
	{
		...
		for (int i = 0; i < axisElementIdentifiers.Count; i++)
		{
			...
			bool flag2 = calibration.selectedElementIdentifierId == val.id;
			bool flag3 = GUILayout.Toggle(flag2, val.name, "Button", GUILayout.ExpandWidth(expand: false));
			if (flag2 != flag3)
			{
		...
		if (calibration.selectedElementIdentifierId >= 0)
		{
			...
			GUILayout.Label("Calibrated Value: " + ((ControllerWithAxes)calibration.joystick).GetAxisById(calibration.selectedElementIdentifierId));

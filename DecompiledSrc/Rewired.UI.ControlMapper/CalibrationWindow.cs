	...
	public void SetJoystick(int playerId, Joystick joystick)
	{
		...
		for (int i = 0; i < ((ControllerWithAxes)joystick).axisCount; i++)
		{
			...
			if (componentInSelfOrChildren != null)
			{
				...
				componentInSelfOrChildren.controllerElementIdentifier = ((ControllerWithAxes)joystick).AxisElementIdentifiers[i];
			}
			else
			{
				...
				{
					componentInSelfOrChildren2.text = ControlMapper.GetLanguage().GetElementIdentifierName((Controller)(object)joystick, ((ControllerWithAxes)joystick).AxisElementIdentifiers[i].id, (AxisRange)0);
				}
			}
			if (num == 0f)
			{

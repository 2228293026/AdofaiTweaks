...
public class Cutscene3 : TaroCutsceneScript
{
	...
	{
		base.Awake();
		shouldDisplay = !GCS.seenCutscene2_1b && Persistence.taroStoryProgress < 2;
	...
	private new void Update()
	{
		...
		for (int i = 0; i < 2; i++)
		{
			midspins[i].transform.eulerAngles = Vector3.forward * (90f + 5f * Mathf.Sin(Time.time * 0.7f * (float)Math.PI + (float)i * 0.5f));
		}
		base.Update();
	}
}
